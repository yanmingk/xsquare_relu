...
plt.savefig('figures/bfgs-optimal-perturb1e-2.png')
...
for m in range(1,range_m):
    for j in range(num_models):
        ...
        #plt.plot(list(range(len(train_losses_m[i]))), np.log10(torch.cat(train_losses_m[i]).numpy()-torch.cat(train_losses_m[i][-1]).numpy()))
        plt.xlabel('iteration')
        plt.ylabel('${\log_10(loss-loss*)}$')
        plt.subplot(3,1,3)
        plt.plot(list(range(len(norm_g_m[i]))), np.log10(torch.cat(norm_g_m[i]).numpy()))
        plt.xlabel('iteration')
        plt.ylabel('${\log_10(grad)}$')
